...
from booktocards.kb import (
    ...
    TokenOrKanjiOrSeqAlreadyExistsForSourceInKbError,
)

# Common to all tests
exp_self_tables = [TableName.TOKENS, TableName.KANJIS, TableName.SEQS, TableName.DOCS]
exp_n_tables = len(exp_self_tables)


@pytest.mark.parametrize(
    "hide_in_add_full_doc_app",
    [True, False],
)
def test_files_are_created_and_reloaded(
    tmp_path, hide_in_add_full_doc_app: bool
) -> None:
    ...
    # Inside the folder, checjk 4 files have been created
    backup_dirname = next(os.walk(path))[1][0]
    backup_path = os.path.join(
        path,
        backup_dirname,
    )
    assert len(os.listdir(backup_path)) == 4
    ...
    kb.create_source_entry(
        source_name="test_doc", hide_in_add_full_doc_app=hide_in_add_full_doc_app
    )
    assert kb.__dict__[TableName.DOCS].shape[0] == 1
    assert "test_doc" in kb.__dict__[TableName.DOCS][ColumnName.SOURCE_NAME].to_list()
    assert (
...
def test_error_when_adding_a_doc_that_already_has_entries(tmp_path):
    # Change path where kb will be saved
    path = tmp_path.resolve()
    # Add doc
    kb = booktocards.kb.KnowledgeBase(kb_dirpath=path)
    kb.create_source_entry(source_name="test_doc")
    doc = "食べる飲む"
    kb.add_doc_from_full_text(
        doc=doc, doc_name="test_doc", drop_ascii_alphanum_toks=False
    )
    # Adding again should fail
    with pytest.raises(TokenOrKanjiOrSeqAlreadyExistsForSourceInKbError):
        kb.add_doc_from_full_text(
            doc=doc, doc_name="test_doc", drop_ascii_alphanum_toks=False
        )
...
def test_set_to_known_works(tmp_path):
    ...
    kb.create_source_entry(source_name="test_doc2")
    kb.add_doc_from_full_text(
...
def test_automatically_set_known_for_new_doc(tmp_path, set_voc_tag):
    ...
    doc = "食べる飲む"
    doc_name = "test_doc"
    kb.create_source_entry(source_name=doc_name)
    kb.add_doc_from_full_text(
    ...
    kb.create_source_entry(source_name="test_doc3")
    kb.add_doc_from_full_text(
...
def test_add_token_with_sequence_error_case_not_in_jamdict(tmp_path) -> None:
    ...
    kb.create_source_entry(source_name=doc_name_error_case)
...
def test_add_token_with_sequence_normal_case(specify_sequence: bool, tmp_path) -> None:
    ...
    # Add a normal token
    kb.create_source_entry(source_name=doc_name_1)
    kb.add_token_with_sequence_to_doc(
        "意を決する",
        sequence=sequence,
        doc_name=doc_name_1,
    ...
    kb.create_source_entry(source_name=doc_name_2)
    kb.add_token_with_sequence_to_doc(
        "焼肉",
        sequence=sequence_2,
        doc_name=doc_name_2,
